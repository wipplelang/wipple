...
type RunResult = { success: boolean; text: string }[];

const fontFamily = "'JetBrains Mono', monospace";

const Playground = () => {
    const [runner, setRunner] = useRefState<typeof import("../../interpreter/pkg") | null>(null);
    useEffect(() => {
        (async () => {
            const runner = await import("../../interpreter/pkg");
            setRunner(runner);
        })();
    }, []);
    ...
    return (
        <div className="flex flex-col bg-gray-50" style={{ height: "100%" }}>
            ...
            >
                ...
                >
                    {result &&
                        result.map(({ text }) => <pre className="whitespace-pre-wrap">{text}</pre>)}
                </div>
            </SplitPane>
