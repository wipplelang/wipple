use crate::{
    ...
    typecheck::infer::{
        ...
        types::{
            ...
            unify::{UnifyOptions, unify_instance, unify_instance_with_options},
        },
    },
    util::WithInfo,
};
...
pub fn resolve_trait(
    query: WithInfo<&Instance>,
    context: &mut ResolveContext<'_>,
) -> Result<WithInfo<Result<Path, Instance>>, WithInfo<QueuedError>> {
    type Candidate = (
        TypeContext,
        WithInfo<(Option<Path>, Instance)>,
        Vec<WithInfo<Instance>>,
    );

    pub fn pick_from_candidates(
        mut candidates: Vec<Candidate>,
        query: WithInfo<&Instance>,
        stack: &[WithInfo<&Instance>],
        _type_context: &mut TypeContext,
    ) -> Result<Option<Candidate>, WithInfo<QueuedError>> {
...
pub fn resolve_custom_error(
    error_info: &Location,
    message_type: &Type,
    context: &mut ResolveContext<'_>,
) -> Option<WithInfo<QueuedError>> {
