...
mod base {
    ...
    impl<D: Driver, Output: 'static> Rule<D, Output> {
            ...
            Output: DefaultFromInfo<D::Info>,
        {
            ...
            self.parse
                .parse(parser, tree, &stack, direction)
                .unwrap_or_else(|| {
                    parser.add_diagnostic(stack.error_expected(
                        WithInfo {
                            info: info.clone(),
        ...
        pub fn parse_option(
            ...
            let result = self.parse.parse(parser, tree, &stack, direction);
            if result.is_none() {
                parser.add_diagnostic(stack.error_expected(
                    WithInfo {
                        info: info.clone(),
                                ...
                                for element in elements {
                                    parser.add_diagnostic(
                                        stack.error_expected(
                                            ...
                                            Direction::After(SyntaxKind::$kind(expected)),
                                        )
                                    );
                                }
    ...
    impl_keyword_rule!(Name(&'static str), contextual_keyword0(), ContextualKeyword);
    impl_keyword_rule!(
        Name(&'static str),
        contextual_keyword1(A),
        ContextualKeyword
    );
    impl_keyword_rule!(
        Name(&'static str),
        contextual_keyword2(A, B),
        ContextualKeyword
    );
    impl_keyword_rule!(
        Name(&'static str),
