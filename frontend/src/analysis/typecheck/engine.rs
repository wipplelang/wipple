use crate::{
    analysis::{self, SpanList},
    TraitId, TypeId, TypeParameterId,
};
use serde::Serialize;
use std::{
    cell::{Cell, RefCell},
...
impl UnresolvedTypeKind {
    pub fn with_usage(self, usage: Option<TypeUsage>) -> UnresolvedType {
        UnresolvedType { usage, kind: self }
    }
}
...
impl TypeKind {
    pub fn with_usage(self, usage: Option<TypeUsage>) -> Type {
        Type { usage, kind: self }
    }
}
        ...
        match &mut self.kind {
            UnresolvedTypeKind::Parameter(param) => {
                *self = substitutions.get(param).cloned().unwrap_or_else(|| {
                    ...
                    UnresolvedType {
                        usage: None,
                        kind: UnresolvedTypeKind::Variable(ctx.new_variable(None)),
                    }
                });
            }
            UnresolvedTypeKind::Function(input, output) => {
