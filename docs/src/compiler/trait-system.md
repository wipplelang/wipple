...
First, the compiler sorts the instances into two groups: one for regular instances and one for `default` instances. The groups are considered in order, so that the compiler will only try `default` instances if no regular instances match. Similarly, when the compiler checks for overlapping instances, this check is done per group, allowing users to define generic `default` instances as well as specific regular instances.
