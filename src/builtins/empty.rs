...
pub(crate) fn init(env: &mut Environment) {
    ...
    let empty_validation = Validation::new(|value: Value, _| {
    ...
    env.add_conformance(Conformance::new(
        ...
        |_, _| Ok(Validation::new(|value: Value, _| Ok(Valid(value.clone())))),
    ));
    ...
    // empty ::= Text
    env.add_conformance(Conformance::new(TraitID::text, empty_validation, |_, _| {
        Ok(Text(String::from("<empty value>")))
    }));
}
