use crate::{
    syntax::{
        Location,
        tokenize::{
            Associativity, Diagnostic, Keyword, ListDelimiter, NonAssociativeOperator, Operator,
            Token, TokenTree, VariadicOperator,
        },
    },
    util::WithInfo,
};
...
impl<'src> TokenTree<'src> {
    ...
    pub fn from_top_level(
        tokens: impl IntoIterator<Item = WithInfo<Token<'src>>>,
    ) -> Option<(
        WithInfo<Self>,            // the root of the tree
        Vec<WithInfo<Diagnostic>>, // any diagnostics produced
    )>
    where
        Location: From<Location>,
    {
        ...
        for token in [first_token].into_iter().chain(tokens.by_ref()) {
            ...
            fn push<'src>(
                info: Location,
                token: &Token<'src>,
                stack: &mut Vec<(Location, TokenTree<'src>)>,
                item: WithInfo<TokenTree<'src>>,
                diagnostics: &mut Vec<WithInfo<Diagnostic>>,
            ) -> bool {
                match stack.last_mut() {
                    ...
                    Some((_, TokenTree::UnresolvedAttribute(contents))) => {
                        if contents.is_some() {
                            ...
                            let item = WithInfo {
                                info: Location::merge(begin_info, info),
            ...
            match token.item {
                ...
                Token::RightParenthesis | Token::RightBracket => {
                    ...
                    let (begin_info, delimiter, expressions) = match stack.pop() {
                        Some((begin_info, TokenTree::List(delimiter, expressions))) => {
                            ...
                            if let Err(diagnostic) = delimiter.match_end(
                                ...
                                diagnostics.push(diagnostic.unboxed());
                            }
                    ...
                    let info = Location::merge(begin_info, info.clone());
                    ...
                    push!(WithInfo {
                        info,
                        item: parse_operators(delimiter, expressions, &mut diagnostics),
                    });
                }
                Token::RightBrace => {
                    ...
                    if let Some(tree) = statements.last_mut() {
                        if matches!(&tree.item, TokenTree::List(_, expressions) if expressions.is_empty())
                        {
                            ...
                            if let Some(first) = expressions.first() {
                                ...
                                tree.info = Location::merge(first.info.clone(), last.info.clone());
                            }

                            tree.item = parse_operators(delimiter, expressions, &mut diagnostics);
                        }
                    }

                    let info = Location::merge(begin_info.clone(), info.clone());
                            ...
                            if let Some(tree) = statements.last_mut() {
                                ...
                                if expressions.is_empty() {
                                    ...
                                    if let Some(first) = expressions.first() {
                                        let last = expressions.last().unwrap();
                                        tree.info =
                                            Location::merge(first.info.clone(), last.info.clone());
                                    }

                                    tree.item =
                                        parse_operators(delimiter, expressions, &mut diagnostics);
                                }
                            }
                        ...
                        match stack.last_mut() {
                            Some((begin_info, TokenTree::Block(statements))) => {
                                ...
                                elements.push(WithInfo {
                                    info: Location::merge(begin_info.clone(), info),
    ...
    pub fn from_inline(
        tokens: impl IntoIterator<Item = WithInfo<Token<'src>>>,
    ) -> Option<WithInfo<Self>>
    where
        Location: From<Location>,
    {
        let (tree, diagnostics) = TokenTree::from_top_level(tokens)?;
...
fn parse_operators<'src>(
    delimiter: WithInfo<ListDelimiter>,
    expressions: Vec<WithInfo<TokenTree<'src>>>,
    diagnostics: &mut Vec<WithInfo<Diagnostic>>,
) -> TokenTree<'src> {
        ...
        AnyOperator::VariadicOperator(operator) => {
            ...
            let elements = groups
                ...
                .map(|group| {
                    ...
                    WithInfo {
                        info,
                        item: parse_operators(delimiter, group, diagnostics),
                    }
                })
                .collect::<Vec<_>>();
...
fn split_at_operator<'src>(
    index: WithInfo<usize>,
    mut expressions: Vec<WithInfo<TokenTree<'src>>>,
    diagnostics: &mut Vec<WithInfo<Diagnostic>>,
) -> (WithInfo<TokenTree<'src>>, WithInfo<TokenTree<'src>>) {
    let info = index.info;
    ...
    match (index > 0, index + 1 < expressions.len()) {
        ...
        (true, true) => {
            ...
            let left_info = Location::merge(
                left.first().unwrap().info.clone(),
                left.last().unwrap().info.clone(),
            );

            let right_info = Location::merge(
            ...
            (
                WithInfo {
                    info: left_info,
                    item: parse_operators(left_delimiter, left, diagnostics),
                },
                WithInfo {
                    info: right_info,
                    item: parse_operators(right_delimiter, right, diagnostics),
                },
            )
        }
