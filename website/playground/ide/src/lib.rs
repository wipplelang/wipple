...
use wipple_playground_runner::SendSyncFuture;
...
#[wasm_bindgen]
pub fn compile() -> Result<JsValue, JsValue> {
    let analysis = get_analysis().ok_or_else(|| JsValue::from_str("program not compiled"))?;
...
pub fn run(handle_io: js_sys::Function, callback: js_sys::Function) -> JsValue {
    ...
    wipple_playground_runner::run(program, handle_io, callback)
}
